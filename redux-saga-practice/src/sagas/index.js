...
function* getUser(action) {
    console.log(action.payload) // action.payload는 fatchUser의 action.payload인 userName
    try {
        const { data } = yield call(api.getUser, action.payload) //api.getUser가 action.payload를 인자로 받는다.
        // 아마 이렇게 안에 .data로 들어올거에요
        yield put({type: 'SET_USER', data})
    } catch(error) {
        yield put({type: 'SET_USER_ERROR', error})
    }
}
